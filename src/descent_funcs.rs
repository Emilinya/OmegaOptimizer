...
pub fn varying_gradient_descent(
	x0: &Array1<f64>, f: &FType, grad_f: &GradFType,
	max_steps: i32, param_path_file: &mut File
) -> GradRes {
    let epsilon = 1e-12;
	...
	for _ in 0..max_steps {
		...
		if v.dot(&v).sqrt() < epsilon {
			return GradRes{val: x, msg: GradMsg::Ok};
        }
		...
		loop {
			...
			let min_idx = match f_diffs.argmin_skipnan() {
				Ok(v) => v,
				Err(_) => return GradRes{val: x, msg: GradMsg::DTErr},
			};
			...
			if dt == 0.0 {
				return GradRes{val: x, msg: GradMsg::DTErr};
			}
        }
	...
	return GradRes{val: x, msg: GradMsg::Timeout};
}

pub fn newton_descent(
	x0: &Array1<f64>, f: &FType, grad_f: &GradFType, hess_f: &HessFType,
	max_steps: i32, param_path_file: &mut File
) -> NewtRes {
	let epsilon = 1e-12;
	...
	match hess_f(x0).cholesky(UPLO::Lower) {
		Ok(_) => { },
		Err(_) => return NewtRes{val: x0.to_owned(), msg: NewtMsg::NotPos},
	};
	...
	for _ in 0..max_steps {
		let v = -grad_f(&x);
		if v.dot(&v).sqrt() < epsilon {
			return NewtRes{val: x, msg: NewtMsg::Ok};
        }

		let inv_hess = match hess_f(&x).inv() {
			Ok(inv) => inv,
			Err(_) => return NewtRes{val: x, msg: NewtMsg::NotInv},
		};

		let x_new = &x + inv_hess.dot(&v);
		if f(&x_new) > f(&x) {
			return NewtRes{val: x, msg: NewtMsg::ErrInc};
		}
	...
	return NewtRes{val: x, msg: NewtMsg::Timeout};
}

pub fn combined_descent(
	x0: &Array1<f64>, f: &FType, grad_f: &GradFType, hess_f: &HessFType,
	print: bool, param_path_file_name: &str
) -> (Array1<f64>, bool) {
	fn is_worse(new_err: f64, old_err: f64) -> bool {
		return new_err.is_nan() || new_err > old_err;
	}
	...
	let mut param_path_file = match File::create(param_path_file_name) {
		Ok(file) => file,
		Err(err) => { panic!("Got error when opening file {}: {}", param_path_file_name, err); },
	};
	if print {
	...
	let (optimal_x, msg) = match newton_descent(
		x0, f, grad_f, hess_f, 50, &mut param_path_file
	) {
		NewtRes{ val, msg } => (val, msg),
	};
	...
	let (improved_x, msg) = match varying_gradient_descent(
		&start_x, f, grad_f, 25, &mut param_path_file
	) {
		GradRes{ val, msg } => (val, msg),
	};
	...
	match msg {
		GradMsg::Ok => return (improved_x, true),
		_ => { },
	}

	// Improve initial x with Newton's method
	let (optimal_x, msg) = match newton_descent(
		&improved_x, f, grad_f, hess_f, 50, &mut param_path_file
	) {
		NewtRes{ val, msg } => (val, msg),
	};
	print_newt_result(&msg, &optimal_x, f(&optimal_x), print);
	...
	if !is_worse(f(&optimal_x), f(&improved_x)) {
		...
		let (improved_x, msg) = match varying_gradient_descent(
			&optimal_x, f, grad_f, 1000, &mut param_path_file
		) {
			GradRes{ val, msg } => (val, msg),
		};
		print_grad_result(&msg, &improved_x, f(&improved_x), print);
		...
		match msg {
			NewtMsg::Ok => return (optimal_x, true),
			_ => return (optimal_x, false),
		}

	} else {
		// Newton increased error, try again with more descent steps
		let (new_improved_x, msg) = match varying_gradient_descent(
			&improved_x, f, grad_f, 1000, &mut param_path_file
		) {
			GradRes{ val, msg } => (val, msg),
		};
		...
		match msg {
			GradMsg::Ok => return (new_improved_x, true),
			_ => { },
		}
	
		// Improve new x with Newton's method
		let (optimal_x, msg) = match newton_descent(
			&new_improved_x, f, grad_f, hess_f, 50, &mut param_path_file
		) {
			NewtRes{ val, msg } => (val, msg),
		};
		print_newt_result(&msg, &optimal_x, f(&optimal_x), print);
