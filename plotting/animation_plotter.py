...
with open("data/datafile.dat", "r") as file:
    match = re.search('(.*?)\((.*?);\s*?(.*?)\)\s*?=\s*?(.*)', file.readline())
...
descents = []
with open("plotting/descent_path.dat", "r") as file:
    descent_mode = ""
    path_list = []
    for line in file:
        line = line.replace("\n", "")
...
min_err, max_err = np.min([np.log(err(descent)) for descent in descents]), np.max([np.log(err(descent)) for descent in descents])
