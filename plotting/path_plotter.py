...
descents = []
with open(parameter_path_file, "r") as file:
    descent_mode = ""
    path_list = []
    for line in file:
        line = line.replace("\n", "")
...
if num_params == 1:
    a_ray = np.linspace(min_vals[0] - ranges[0]*0.5, max_vals[0] + ranges[0]*0.5, 100)
    ...
    plt.colorbar(label=f"$\\ln(E({sp.latex(sym_params[0])}, {sp.latex(sym_params[1])}))$")
