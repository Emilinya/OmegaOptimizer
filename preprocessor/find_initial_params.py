...
class ParameterInputBox:
	def __init__(self, name, font, x, y, w, h, value=1):
		...
		self.font = font
		self.font_width = self.font.render("1234567890", True, (0, 0, 0)).get_width() / 10
		self.w = w
	...
	def set_activation(self, activation):
		...
		if is_number(self.value):
			...
			self.color = self.active_error_color if self.active else self.incactive_error_color
...
def find_initial_params():
	...
	with open("preprocessor/initial_params.dat", "r") as paramfile:
		text = paramfile.read()
		if text != "":
	...
	tutorial_font_size = 32

	tutorial_text_1 = font.render(
		"Change parameter values to",
		True, (0, 0, 0)
	)
	tutorial_text_2 = font.render(
		"approximate the corect parameters",
		True, (0, 0, 0)
	)
	tutorial_height = tutorial_text_1.get_height() + tutorial_text_2.get_height()

	window = pg.display.set_mode((figsize[0]*dpi, figsize[1]*dpi + parameter_space + tutorial_height), DOUBLEBUF)
	...
	plot = Plot(
		(0, tutorial_height + parameter_space - 40), x_ray, y_ray, f, [1]*len(parameters),
	...
	inputs = [
		ParameterInputBox(
			parameter, font, max_width, 10 + tutorial_height + parameter_padding*(i+1) + 32*i, 100, 32,
